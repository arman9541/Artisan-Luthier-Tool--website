...
// === RATE LIMITERS ===
app.use(rateLimit({ windowMs: 15 * 60 * 1000, max: 100 })); // general
...
app.post('/api/cart/add', (req, res) => {
  ...
  if (!PRODUCTS[productId]) {
    console.error(`[CART ERROR] Invalid product ID: ${productId}`);
...
app.post('/create-checkout-session', checkoutLimiter, async (req, res) => {
  try {
    ...
    for (const [id, qty] of Object.entries(cart)) {
      ...
      if (!product) {
        console.error(`[CHECKOUT] Invalid product ID in cart: ${id}`);
        return res.status(400).json({ error: `Invalid product in cart: ${id}` });
      }

      const unit_amount = Math.round(Number(product.price) * 100);
      if (isNaN(unit_amount) || unit_amount <= 0) {
        console.error(`[CHECKOUT] Invalid price for product ${id}: ${product.price}`);
        return res.status(400).json({ error: `Invalid price for product: ${id}` });
      }
    ...
    console.log('[CHECKOUT] Line items prepared:', line_items);

    // ===== URL BUILD + DEBUG =====
    const baseUrlRaw = process.env.BASE_URL || '';
    const baseUrl = baseUrlRaw.trim().replace(/\/+$/, ''); // remove spaces + trailing slash
    ...
    console.log('BASE_URL RAW:', JSON.stringify(baseUrlRaw));
    console.log('BASE_URL CLEAN:', JSON.stringify(baseUrl));
