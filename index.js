...
// Required for mobile browsers behind proxies (Render/Railway)
...
// MOBILE FIX: Optimized session cookie settings
app.use(session({
  secret: process.env.SESSION_SECRET,
  resave: false,
  saveUninitialized: false,
  name: 'artisan_session', // Custom name can help avoid some mobile browser defaults
  cookie: {
    // secure MUST be true for sameSite: 'none' to work on mobile browsers
...
app.post('/create-checkout-session', checkoutLimiter, async (req, res) => {
  try {
    ...
    const sessionObj = await stripe.checkout.sessions.create({
      ...
      line_items,
      mode: 'payment',
      // MOBILE FIX: Collect billing address to enable Apple/Google Pay on phones
