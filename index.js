...
app.post('/create-checkout-session', checkoutLimiter, async (req, res) => {
  try {
    ...
    // ===== Detect if Popot Rouge is in cart =====
    const containsPopot = Object.keys(cart).includes('popot-rouge');
    ...
    if (containsPopot) {
      ...
      sessionConfig.shipping_options = [
        { shipping_rate: 'shr_1T22Xl5uGKgjYm0qnVNVe8oT' }
      ];

      console.log('[CHECKOUT] Popot Rouge detected → US shipping only');
      ...
      console.log('[CHECKOUT] Standard product → International allowed');
    }
