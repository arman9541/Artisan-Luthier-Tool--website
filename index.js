...
// ======= TRUST PROXY =======
// Crucial for Render/Mobile to trust the HTTPS headers for cookies
...
// ======= SESSION (FIXED FOR MOBILE) =======
app.use(session({
  secret: process.env.SESSION_SECRET,
  resave: false,
  saveUninitialized: true, // Changed to true so mobile gets a cookie on first load
  name: 'artisan_session',
  cookie: {
    secure: isProd,           // Required true for SameSite: 'none'
    httpOnly: true,
    sameSite: isProd ? 'none' : 'lax', // 'none' allows cross-domain cookies on mobile
...
app.post('/api/cart/add', (req, res) => {
  ...
  // FIX: Explicitly save session before sending response
...
app.use((err, req, res, next) => {
  if (err.code === 'EBADCSRFTOKEN') {
      console.warn('CSRF Error:', req.headers['csrf-token'] ? 'Token Present' : 'Token Missing');
