    ...
    contentSecurityPolicy: {
        directives: {
            ...
            connectSrc: [
                ...
                FRONTEND_ORIGIN,
                "https://artisan-luthier-tool-website-1.onrender.com"
            ]
        }
    }
}));
...
// ======= SESSIONS (Enhanced for Debugging) =======
app.use(session({
...
// ======= CSRF WITH LOGGING =======
app.use(csrf({
    value: req => req.headers['csrf-token'] || req.headers['CSRF-Token'] || req.body._csrf || req.query._csrf,
    //                       ^^^^^^^^^^^^^ lowercase  ^^^^^^^^^^^^^ capitalized (both supported)
    ignoreMethods: ['GET', 'HEAD', 'OPTIONS']
}));

// Add this custom middleware right AFTER the CSRF middleware to debug
app.use((req, res, next) => {
...
// Add catch-all route for SPA (add BEFORE error handler)
app.get('*', (req, res) => {
